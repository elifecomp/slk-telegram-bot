...
case $MENU_CHOICE in
  ...
  2)
    ...
    if [ -d "/opt/SLV_Bot" ]; then
        echo "⚠️ Папка /opt/SLV_Bot уже существует"
    ...
    git clone https://elifecomp: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@github.com/elifecomp/slk-telegram-bot.git /opt/SLV_Bot
...
ENVEOF
    if [ "$HAS_PANEL2" = "y" ]; then
      echo "XUI2_PANEL_URL=$XUI2_PANEL_URL" >> /opt/SLV_Bot/.env
      echo "XUI2_API_TOKEN=$XUI2_API_TOKEN" >> /opt/SLV_Bot/.env
      echo "XUI2_VERIFY_SSL=False" >> /opt/SLV_Bot/.env
    fi
    ...
    echo ""
    echo "✅ Бот установлен и настроен!"
    echo "📝 Напишите /start в боте"
    ;;
  3)
